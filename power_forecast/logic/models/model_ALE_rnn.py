...
def train_or_load_model_lstm(
    ...
    if train_new_model:
        model = initialize_model_lstm(
            input_shape=X_tr.shape[1:], output_length=OUTPUT_LENGTH
        )
...
print(f"MAE  (real scale): {mae_real:.2f} MW")
print(f"MSE  (real scale): {mse_real:.2f} MW²")
print(f"RMSE (real scale): {rmse_real:.2f} MW")
